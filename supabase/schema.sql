...
create policy "Users can view own saved spots." on public.saved_spots for select using ( auth.uid() = user_id );
create policy "Users can save spots." on public.saved_spots for insert with check ( auth.uid() = user_id );
create policy "Users can unsave spots." on public.saved_spots for delete using ( auth.uid() = user_id );
