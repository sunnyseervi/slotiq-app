        ...
        {filtered.length === 0 ? (
          ...
          <div className="flex flex-col gap-3 p-4 pb-24">
            {filtered.map(b => {
              ...
              const isParking = b.listing_type === 'parking'
              ...
              return (
                <div key={b.id} className="card overflow-hidden">
                  ...
                  >
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl flex-shrink-0 ${isParking ? 'bg-orange-50 dark:bg-orange-950' : 'bg-blue-50 dark:bg-blue-950'}`}>
                      {b.listing_emoji}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="font-extrabold text-sm text-gray-900 dark:text-white line-clamp-1 mb-1">{b.listing_name}</div>
                      <div className="text-xs text-muted flex items-center gap-1">
                        📅 {formatDate(b.booking_date)} · {formatTime(b.start_time)} – {formatTime(b.end_time)}
                      </div>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <div className="text-base font-extrabold text-primary mb-1.5">{formatInr(b.total_cost)}</div>
                      <span className={`text-[10px] font-extrabold px-2 py-1 rounded-full ${s.cls}`}>{s.label}</span>
                    </div>
                  </div>
                  ...
                  {isExp && (
                    <div className="px-4 pb-4 border-t border-gray-100 dark:border-gray-700 pt-3 animate-fadeIn">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="font-extrabold text-primary tracking-widest text-sm">{b.confirmation_code}</span>
                      </div>
                      {b.vehicle_plate && (
                        <div className="flex items-center gap-2 mb-3 text-xs">
                          <span className="text-muted">Vehicle</span>
                          <span className="font-bold text-gray-700 dark:text-gray-200">🚗 {b.vehicle_plate}</span>
                        </div>
                      )}
